class CPUParametersGenerator < WikiGenerator
  def generate_content(_options)
    table_columns = ['Installation date', 'Manufacturing date', 'Site', 'Cluster', 'CPU Family', 'CPU Version',
                     'Microarchitecture', 'Frequency', 'Server type', 'HT enabled', 'Turboboost enabled', 'P-State driver', 'C-State driver']
    table_data = []
    ...
    global_hash['sites'].sort.to_h.each do |site_uid, site_hash|
      site_hash.fetch('clusters', {}).sort.to_h.each do |cluster_uid, cluster_hash|
        ...
        cpu_freq = node_hash['processor']['clock_speed'] / 1_000_000_000.0
