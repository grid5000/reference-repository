...
class OarPropertiesGenerator < WikiGenerator
  ...
  @@categories = {
    'Job-related properties' => %w[besteffort deploy production cluster_priority max_walltime],
    'Hierarchy' => %w[chassis cluster cpu cpuset core disk diskpath gpu gpudevice host
                      slash_16 slash_17 slash_18 slash_19 slash_20 slash_21 slash_22 switch subnet_address subnet_prefix vlan nodeset],
    'Hardware' => %w[gpu_model gpu_count gpu_mem gpu_compute_capability memnode memcore memcpu
                     disktype disk_reservation_count ib_rate ib_count ib opa_rate opa_count eth_rate eth_count eth_kavlan_count cpufreq cputype cpucore cpuarch core_count cpu_count thread_count virtual mic],
    'Miscellaneous' => %w[wattmeter nodemodel network_address ip type expiry_date comment
                          maintenance exotic]
  }
  ...
  def get_value_type(prop, values)
    return @@properties[prop]['value_type'] if @@properties[prop]['value_type']
  ...
  def generate_content(_options)
    ...
    oar_properties = {}
    props.sort.to_h.each do |_site, site_props|
      site_props.sort.to_h.each do |_type, type_props|
        type_props.sort.to_h.each do |_node_uid, node_props|
          node_props.sort.to_h.each do |property, value|
            ...
            oar_properties[property]['values'].sort! do |a, b|
              (a && a.to_s || '') <=> (b && b.to_s || '')
            end
          end
        end
    ...
    oar_properties.sort.to_h.each do |prop, prop_hash|
    ...
    unless oarapi_properties.empty?
    ...
    @@categories.sort.to_h.each do |cat, cat_properties|
      @generated_content += MW.heading(cat, 2) + MW::LINE_FEED
      cat_properties.sort.each do |property|
        values = begin
          oar_properties[property]['values']
