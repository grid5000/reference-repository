...
class G5KHardwareGenerator < WikiGenerator
  def generate_content(_options)
    ...
    @generated_content += '<div class="sitelink">Hardware: [[Hardware|Global]] | ' + G5K::SITES.map { |e|
  ...
  def generate_totals
    ...
    @global_hash['sites'].sort.to_h.each do |site_uid, site_hash|
      site_hash.fetch('clusters', {}).sort.to_h.each do |_cluster_uid, cluster_hash|
        cluster_hash['nodes'].sort.to_h.each do |node_uid, node_hash|
          ...
          proc_desc = "#{model}"
          proc_desc += " #{version}" if version != 'Unknown'
          ...
          key = [vendor, { text: proc_desc, sort: get_date(microarchitecture) + ', ' + proc_desc.to_s },
                 { text: microarchitecture || ' ', sort: get_date(microarchitecture) + ', ' + microarchitecture.to_s }, { text: architecture, sort: architecture }]
          ...
          # HPC Networks
          interfaces = node_hash['network_adapters'].select do |v|
            v['enabled'] and
          ...
          net_interconnects = interfaces.each_with_object(Hash.new(0)) do |v, h|
            h[v] += 1
          end
          net_interconnects.sort_by { |k, _v| k.first[:sort] }.each do |k, v|
          ...
          net_models = interfaces.each_with_object(Hash.new(0)) do |v, h|
            h[v] += 1
          end
          # Sort by interface type (eth or IB) and then by driver
          net_models.sort_by { |k, _v| [k.first[:sort], k[1][:sort]] }.each do |k, v|
            k.first[:text] = k.first[:text] + '*' if k.first[:text] == 'FPGA/Ethernet'
          ...
          # NVMe SSD models
          ssd = node_hash['storage_devices'].select do |v|
            v['storage'] == 'SSD'
          end.map do |v|
            t = v['alt_model_name'] || v['model'] || 'N/A'
            # Add vendor if not already there
            t = "#{v['vendor']} #{t}" unless t.downcase.start_with?(v['vendor'].downcase)
            [
              {
          ...
          ssd_models = ssd.each_with_object(Hash.new(0)) do |v, h|
            h[v] += 1
          end
          ssd_models.sort_by { |k, _v| k.first[:sort] }.each do |k, v|
          ...
          { GPU: node_hash.has_key?('gpu_devices') ? node_hash['gpu_devices'] : {},
            ...
            FPGA: if node_hash.has_key?('other_devices')
                    node_hash['other_devices'].select do |k, _|
                      k =~ /^fpga\d+$/
                    end
                  else
                    {}
                  end }.each do |acc_type, acc_data|
            acc_data.each_value do |acc|
              ...
              key = [vendor,
                     ...
                     { text: compute_capability,
                       sort: (compute_capability == 'N/A' ? 0 : compute_capability.gsub('.', ', ')) }]
              init(data, 'acc_models', key)
              data['acc_models'][key][site_uid] += 1

              next unless acc_type == :GPU
    ...
    table_columns = %w[Vendor Type] + sites + ['Accelerators total']
    generated_content += MW.generate_table(table_options, table_columns, get_table_data(data, 'acc_families'))
    table_columns = ['Vendor', 'Type', 'Model', 'Microarch', 'Memory',
                     'Compute capability'] + sites + ['Accelerators total']
    ...
    table_columns = %w[Type Driver Model] + sites + ['Cards total']
  ...
  def get_table_data(data, key)
    ...
    index = 0
    k0 = 0
    data[key].sort_by do |k, _v|
      k.map { |c| c.is_a?(Hash) ? c[:sort] : c }
      ...
      table_data << k.map { |e|
        if e.is_a?(Hash)
          "data-sort-value=\"#{e[:sort]}\"|#{e[:text]}"
        else
          "data-sort-value=\"#{index.to_s.rjust(3,
                                                '0')}\"|#{e}"
        end
      } +
                    elts.map { |e| e.is_a?(Hash) ? "data-sort-value=\"#{e[:sort]}\"|#{e[:text]}" : e }
    ...
    global_hash['sites'].sort.to_h.each do |site_uid, site_hash|
      site_hash.fetch('clusters', {}).sort.to_h.each do |cluster_uid, cluster_hash|
        ...
        cluster_hash.fetch('nodes').sort.to_h.each do |node_uid, node_hash|
          ...
          hdd_t = if hdds.count == 0
                    '0'
                  else
                    hdds.count.to_s + ' (' + hdds.map { |d|
                      G5K.get_size(d['size'], 'metric') +
                        (!d['reservation'].nil? && d['reservation'] ? '[[Disk_reservation|*]]' : '')
                    }.join(', ') + ')'
                  end
          ssds = other.select { |d| d['storage'] == 'SSD' }
          ssd_t = if ssds.count == 0
                    '0'
                  else
                    ssds.count.to_s + ' (' + ssds.map { |d|
                      G5K.get_size(d['size'], 'metric') +
                        (!d['reservation'].nil? && d['reservation'] ? '[[Disk_reservation|*]]' : '')
                    }.join(', ') + ')'
                  end
          nodes_data << { 'uid' => node_uid,
                          'data' => { 'main' => maindisk_t, 'hdd' => hdd_t, 'ssd' => ssd_t,
                                      'reservation' => reservable_disks } }
        end
        ...
        nd.each do |data, nodes|
          ...
          nodesetname = if nd.length == 1
                          cluster_uid
                        else
                          cluster_uid + '-' + G5K.nodeset(nodes.map { |n| n['uid'].split('-')[1].to_i })
                        end
          table_data << [
  ...
  def generate_interfaces
    ...
    @global_hash['sites'].sort.to_h.each do |site_uid, site_hash|
      site_hash.fetch('clusters', {}).sort.to_h.each do |cluster_uid, cluster_hash|
        ...
        cluster_hash.fetch('nodes').sort.to_h.each do |node_uid, node_hash|
          ...
          next unless node_hash['network_adapters']

          node_interfaces = node_hash['network_adapters'].select do |v|
          ...
          interfaces['details'] = node_interfaces.map do |v|
            v['name'] + ' (' + G5K.get_rate(v['rate']) + ')'
          end.sort.join(', ')
          queues = cluster_hash['queues'] - %w[admin default testing]
          interfaces['queues'] =
            (queues.nil? || (queues.empty? ? '' : queues[0] + G5K.pluralize(queues.count, ' queue')))
        ...
        network_interfaces.sort.to_h.each do |num, interfaces|
          table_data << [
    ...
    table_options = 'class="wikitable sortable" style="text-align: center;"'
    table_columns = ['Site', 'Cluster', 'Nodes', '25G interfaces', '10G interfaces', '1G interfaces',
