...
class SiteHardwareGenerator < WikiGenerator
  ...
  def self.generate_header_summary(sites_hash)
    ...
    if store.key?('default') && store.key?('production')
      ...
      summary += print_header_summary('=== Experimentation ressources ===', 'default', store)
      ...
      ## 2st inner table
      summary += MW::TABLE_START + ' width="100%"' + MW::LINE_FEED
      summary += print_header_summary('=== Computation ressources ===', 'production', store)
      summary += MW::TABLE_END + MW::LINE_FEED
  ...
  def self.print_header_summary(title, queue, store)
    output = "#{title}\n"
    output += store[queue][:sites].size > 1 ? "* #{store[queue][:sites].size} sites\n":''
    ...
    output += store[queue][:gpus] > 0 ? "* #{store[queue][:gpus]} GPUs\n":''
    output += store[queue][:gpus] > 0 ? "* #{store[queue][:gpus_cores]} GPUs cores\n":''
    output += "* #{G5K.get_size(store[queue][:ram])} RAM"
    output += store[queue][:pmem] > 0 ? " + #{G5K.get_size(store[queue][:pmem])} PMEM\n":"\n"
    output += (store[queue][:ssds] > 0 ? "* #{store[queue][:ssds]} SSDs and ":"* ") + "#{store[queue][:hdds]} HDDs on nodes (total: #{G5K.get_size(store[queue][:storage_space], 'metric')})\n"
