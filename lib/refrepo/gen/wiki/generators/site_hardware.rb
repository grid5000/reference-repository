...
class SiteHardwareGenerator < WikiGenerator
  ...
  def generate_content(_options)
    ...
    if has_omni_path
      asterisks << "''***: OPA (Omni-Path Architecture) is currently not supported on Debian 12 environment''"
    end

    @generated_content = "__NOEDITSECTION__\n" +
                         "{{Portal|User}}\n" +
                         '<div class="sitelink">Hardware: [[Hardware|Global]] | ' + G5K::SITES.map { |e|
                                                                                      "[[#{e.capitalize}:Hardware|#{e.capitalize}]]"
                                                                                    }.join(' | ') + "</div>\n" +
  ...
  def self.generate_all_clusters
    ...
    G5K::SITES.each do |site|
      table_columns = generate_summary_data(site, true)[0]
      table_data += generate_summary_data(site, true)[1]
    end
  ...
  def self.generate_split_tables(table_columns, table_data, with_site, asterisks)
    ...
    [
      ...
      next unless table_data.select { |row| row[column] =~ regexp_select and row[column] !~ regexp_reject }.length > 0

      output += "#{title}\n"
      output += MW.generate_table('class="wikitable sortable"', table_columns, table_data.select do |row|
        row[column] =~ regexp_select and row[column] !~ regexp_reject
      end) + "\n"
  ...
  def self.print_header_summary(title, queue, store)
    ...
    # tflops
    tflops = format('%.1f', store[queue][:flops].to_f / (10**12))
    output += "* #{tflops} TFLOPS (excluding GPUs)\n"
    output
  end

  def self.generate_summary(site, with_sites, asterisks)
    table_columns, table_data = generate_summary_data(site, with_sites)
  ...
  def self.generate_summary_data(site, with_sites)
    ...
    site_accelerators = 0
    hardware[site].sort.to_h.each do |_cluster_uid, cluster_hash|
      site_accelerators += cluster_hash.select { |_k, v| v['accelerators'] != '' }.count
    end

    hardware[site].sort_by do |cluster_uid, _cluster_hash|
      split_cluster_node(cluster_uid)
    end.to_h.each do |cluster_uid, cluster_hash|
      cluster_nodes = cluster_hash.keys.flatten.count
      queue = cluster_hash.map { |_k, v| v['queue'] }.first
      access_conditions = []
      if %w[production abaca].include?(queue)
        access_conditions << '<b>[[Grid5000:UsagePolicy#Rules_for_the_production_queue|abaca]]</b>&nbsp;queue'
      elsif queue != ''
        access_conditions << "<b>#{queue}</b>&nbsp;queue"
      end
      access_conditions << "<b>[[Exotic##{site.capitalize}:_#{cluster_uid}|exotic]]</b>&nbsp;job&nbsp;type" if cluster_hash.map do |_k, v|
        v['exotic']
      end.first
      table_columns = []
      table_columns << (if with_sites == true
                          [{ attributes: 'rowspan=2',
                             text: 'Site' }]
                        else
                          []
                        end) + [{ attributes: 'rowspan=2', text: 'Cluster' },
                                { attributes: 'rowspan=2', text: 'Access Condition' }, { attributes: 'rowspan=2', text: 'Date of arrival' }, { attributes: 'rowspan=2', text: 'Manufacturing date' }, { attributes: 'data-sort-type="number" rowspan=2', text: 'Nodes' }, { attributes: 'colspan=4', text: 'CPU' }, { attributes: 'data-sort-type="number" rowspan=2', text: 'Memory' }, { attributes: 'data-sort-type="number" rowspan=2', text: 'Storage' }, { attributes: 'data-sort-type="number" rowspan=2', text: 'Network' }] + (if site_accelerators.zero? && with_sites == false
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          []
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        else
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          [{
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            attributes: 'rowspan=2', text: 'Accelerators'
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          }]
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        end)
      table_columns << [{ attributes: 'data-sort-type="number"', text: '#' }, 'Name',
                        { attributes: 'data-sort-type="number"', text: 'Cores' }, 'Architecture']
      ...
      table_data << (with_sites == true ? ["[[#{site.capitalize}:Hardware|#{site.capitalize}]]"] : []) + [
        ...
        'data-sort-value="' + sort_data(data,
                                        'memory_size') + '"|' + sort_data(data,
                                                                          'ram_size') + (if !data['pmem_size'].nil?
                                                                                           " + #{cell_data(
                                                                                             data, 'pmem_size'
                                                                                           )} [[PMEM]]"
                                                                                         else
  ...
  def self.get_queue_drawgantt_url(site, queue)
    ...
    url += if %w[production abaca].include?(queue)
             'drawgantt-svg-prod/'
           # elsif (queue == 'testing')
  ...
  def self.generate_description(site)
    ...
    hardware[site].group_by do |_cluster_uid, cluster_hash|
      ...
      clusters.sort_by do |cluster_uid, _cluster_hash|
        ...
        access_conditions << "[[Exotic##{site.capitalize}:_#{cluster_uid}|exotic]] job type" if cluster_hash.map do |_k, v|
          v['exotic']
        end.first

        cluster_drawgantt_url = get_queue_drawgantt_url(site, queue) + "?filter=#{cluster_uid}%20only"
        text_data << ["\n== [#{cluster_drawgantt_url} #{cluster_uid}] ==\n"]
        text_data << ["'''#{cluster_nodes} #{G5K.pluralize(cluster_nodes,
                                                           'node')}, #{cluster_cpus} #{G5K.pluralize(cluster_cpus,
                                                                                                     'cpu')}, #{cluster_cores} #{G5K.pluralize(cluster_cores,
                                                                                                                                               'core')}" + (subclusters == true ? ",''' split as follows due to differences between nodes " : "''' ") + "([https://public-api.grid5000.fr/stable/sites/#{site}/clusters/#{cluster_uid}/nodes.json?pretty=1 json])"]
        ...
        if %w[production abaca].include?(queue)
          ...
          max_walltime_per_node = nodes.map do |node_name, node|
            [node['supported_job_types']['max_walltime'] / 3600, node_name.sub("#{cluster_uid}-", '').to_i]
          end
          walltime_breakout = {}
          max_walltime_per_node.each do |e|
          ...
          walltime_breakout.sort.each do |hours, allnodes|
            prev = allnodes[0]
            range = allnodes.sort.slice_before do |node|
              prev2 = prev
              prev = node
              prev2 + 1 != node
            end.map { |b, *, c| c ? "#{b}-#{c}" : b.to_s }
        ...
        cluster_hash.sort.to_h.each_with_index do |(num, h), i|
          if subclusters
            ...
            text_data << ["; #{cluster_uid}-#{G5K.nodeset(num)} (#{subcluster_nodes} #{G5K.pluralize(subcluster_nodes,
                                                                                                     'node')}, #{subcluster_cpus} #{G5K.pluralize(subcluster_cpus,
                                                                                                                                                  'cpu')}, #{subcluster_cores} #{G5K.pluralize(
                                                                                                                                                    subcluster_cores, 'core'
                                                                                                                                                  )})"]
          end
          ...
          hash = {}
          hash['Access condition'] = access_conditions.join(', ') unless access_conditions.empty?
          hash.merge!({
                        'Model' => h['model'],
        ...
        text_data << "\n'''Note:''' This cluster is defined as exotic. Please read the '''[[Exotic##{site.capitalize}:_#{cluster_uid}|exotic]]''' page for more information.<br/>" if cluster_hash.map do |_k, v|
          v['exotic']
        end.first
      end
    end
...
def partition(cluster_hash)
  data = {}
  h1 = {}
  cluster_hash.sort.to_h.each do |_num2, h2|
    h2.each_key do |k|
      h1[k] = []
      cluster_hash.sort.to_h.each do |num3, h3|
        if h1[k].map { |e| e['value'] }.include?(h3[k])
          num = h1[k].select { |e| e['value'] == h3[k] }.first['num']
          h1[k].delete_if { |e| e['value'] == h3[k] }
          h1[k] += [{ 'num' => num + num3, 'value' => h3[k], 'cell' => "#{G5K.nodeset(num + num3)}: #{h3[k]}",
                      'sort' => "#{h3[k]}" }]
        else
          h1[k] += [{ 'num' => num3, 'value' => h3[k], 'cell' => "#{G5K.nodeset(num3)}: #{h3[k]}",
                      'sort' => "#{h3[k]}" }]
        end
      end
...
def gpu_model_description(device_hash, long_name)
  ...
  if long_name
    ...
    unless GPURef.is_gpu_supported?(device_hash)
      description = '<s>' + description + "</s><br>''not supported by Grid'5000 default environments''"
    end
  elsif !GPURef.is_gpu_supported?(device_hash)
...
def get_hardware(sites)
  ...
  hardware = {}
  global_hash['sites'].sort.to_h.select do |site_uid, _site_hash|
    sites.include?(site_uid)
  end.each do |site_uid, site_hash|
    hardware[site_uid] = {}
    site_hash.fetch('clusters', {}).sort.to_h.each do |cluster_uid, cluster_hash|
      hardware[site_uid][cluster_uid] = {}
      cluster_hash.fetch('nodes').sort.each do |node_uid, node_hash|
        next if node_hash['status'] == 'retired'

        # map model to vendor (eg: {'SAS5484654' => 'Seagate', 'PX458' => 'Toshiba' ...}
        hard = {}
        queue = cluster_hash['queues'] - %w[admin default]
        ...
        hard['cpus_per_node_str'] =
          hard['cpus_per_node'].to_s + '&nbsp;' + G5K.pluralize(hard['cpus_per_node'], 'CPU') + '/node'
        hard['cores_per_cpu'] = node_hash['architecture']['nb_cores'] / hard['cpus_per_node']
        hard['cores_per_cpu_str'] =
          hard['cores_per_cpu'].to_s + '&nbsp;' + G5K.pluralize(hard['cores_per_cpu'], 'core') + '/CPU'
        ...
        hard['num_processor_model'] =
          (hard['cpus_per_node'] == 1 ? '' : "#{hard['cpus_per_node']}&nbsp;x&nbsp;") + (exotic_archname ? "#{exotic_archname}&nbsp;" : '') + hard['processor_model'].gsub(
            ' ', '&nbsp;'
          )
        hard['processor_description'] =
          "#{hard['processor_model']} (#{hard['microarchitecture']}), #{hard['architecture']}#{hard['processor_freq'] ? ', ' + hard['processor_freq'] : ''}, #{hard['cpus_per_node_str']}, #{hard['cores_per_cpu_str']}"
        hard['ram_size'] = G5K.get_size(node_hash['main_memory']['ram_size'])
        hard['memory_size'] = node_hash['main_memory']['ram_size']
        unless node_hash['main_memory']['pmem_size'].nil?
          hard['pmem_size'] =
            G5K.get_size(node_hash['main_memory']['pmem_size'])
        end
        # hard['memory_size'] += node_hash['main_memory']['pmem_size'] unless node_hash['main_memory']['pmem_size'].nil?
        storage = node_hash['storage_devices'].sort_by! do |d|
          d['id']
        end.map do |i|
          { 'size' => i['size'], 'tech' => i['storage'],
            'reservation' => i['reservation'].nil? ? false : i['reservation'] }
        end
        hard['storage'] = storage.each_with_object(Hash.new(0)) do |data, counts|
          counts[data] += 1
        end.to_a
        ...
        hard['storage_size'] = # round to GB to avoid small differences within a cluster
          storage.inject(0) do |sum, v|
            sum + (v['size'].to_f / 2**30).floor
          end.to_s
        storage_description = node_hash['storage_devices'].sort do |a, b|
          a['device'] <=> b['device']
        end.map do |v|
          {
            'device' => v['device'],
        ...
        has_reservable_disks = false
        hard['storage_description'] = storage_description.sort_by! { |d| d['id'] }.map do |e|
        ...
        network = node_hash['network_adapters'].select do |v|
          v['management'] == false &&
        ...
        hard['used_networks'] = network.select do |e|
          e['used'] == true
        end.each_with_object(Hash.new(0)) do |data, counts|
          counts[data] += 1
        end.to_a.sort_by do |e|
          e[0]['rate'].to_f
        end.map do |e|
          get_network_info(e, false)
        end.join('+&nbsp;')

        hard['network_throughput'] = # round to Mbps
          network.select do |e|
            e['used'] == true
          end.inject(0) do |sum, v|
            sum + (v['rate'].to_f / 10**6).floor
          end.to_s
        network_description = node_hash['network_adapters'].select do |v|
          v['management'] == false &&
        ...
        hard['network_description'] = network_description.map do |e|
          desc = []
          desc << if e['name'].nil? or e['name'] == e['device']
                    e['device']
                  else
                    e['device'] + '/' + e['name']
                  end
          ...
          unless e['unwired'] and e['unavailable_for_experiment']
            desc << 'configured rate: ' + (e['unwired'] ? 'n/c' : G5K.get_rate(e['rate']))
          end
          unless e['model'] == 'N/A N/A' and e['unavailable_for_experiment'] # don't include interface model if not available
            e['model'] = 'N/A' if e['model'] == 'N/A N/A'
            desc << 'model: ' + e['model']
          end
          desc << 'driver: ' + e['driver'] if e['driver']
          desc << 'SR-IOV enabled' if e['sriov'] and !e['unavailable_for_experiment']
          ...
          if e['no_kavlan']
            ...
            s += ' [[Advanced_KaVLAN#A_simple_multi_NICs_example|(multi NICs example)]]' unless nic_c.zero?
            nic_c += 1
          end
          s = "\n* " + s
          s
        ...
        hard['accelerators'] = if hard['gpu_str'] != ''
                                 hard['gpu_str'] + (hard['mic_str'] != '' ? ' ; ' + hard['mic_str'] : '')
                               else
                                 hard['mic_str']
                               end
        hard['accelerators'] += hard['fpga_str'] if hard['fpga_str'] != ''

        hard['accelerators_long'] = if hard['gpu_str_long'] != ''
                                      hard['gpu_str_long'] + (hard['fpga_str'] != '' ? ' ; ' + hard['fpga_str'] : '')
                                    else
                                      hard['fpga_str']
                                    end
