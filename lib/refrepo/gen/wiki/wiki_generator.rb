...
class WikiGenerator
  ...
  def remove_page_creation_date(content)
    content.gsub(%r{''<small>Last generated from the Grid'5000.+</small>''}, '')
  end

  def generated_date_string
    commit = `git show --oneline -s`.split(' ').first
    date = Time.now.strftime('%Y-%m-%d')
    "Last generated from the Grid'5000 Reference API on #{date} ([https://gitlab.inria.fr/grid5000/reference-repository/commit/#{commit} commit #{commit}])"
  end
  ...
  def diff_files
    ret = true
    @files.each do |file|
      next unless file['content-type'] == 'text/plain'
  ...
  def exec(options)
    ...
    login(options) if options[:diff] || options[:update]
    if options[:diff]
