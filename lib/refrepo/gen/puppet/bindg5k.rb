...
class DNS::Zone::RR::A
  def dump
    max_pad = 30
    "#{@label.ljust(max_pad)} IN A #{' ' * 6 + @address}"
  end
end
...
class DNS::Zone::RR::AAAA
  def dump
    max_pad = 30
    "#{@label.ljust(max_pad)} IN AAAA #{' ' * 6 + @address}"
  end
end
...
class DNS::Zone::RR::CNAME
  def dump
    max_pad = 30
    @label.ljust(max_pad) + ' IN CNAME ' + ' ' * 6 + @domainname
  end
end
...
class DNS::Zone::RR::NS
  def dump
    max_pad = 30
    @label.ljust(max_pad) + ' IN NS ' + ' ' * 6 + @nameserver
  end
end
...
class DNS::Zone::RR::MX
  def dump
    max_pad = 30
    @label.ljust(max_pad) + ' MX ' + @priority.to_s + ' ' * 6 + @exchange
  end
end
...
class DNS::Zone
  attr_accessor :file_path, :site_uid, :header, :soa, :ns_list, :mx, :at, :include
  ...
  def dump
    ...
    @records.each do |record|
      content << "\n; #{record.type} records" if record.type != last_type
      content << record.dump
      last_type = record.type
    end
    get_header + content.join("\n") << "\n"
  end
end

def get_servers_records(site)
  records = []
  site['servers'].sort.each do |server_uid, server|
    next if server['network_adapters'].nil?

    server['network_adapters'].each do |net_uid, net|
      ...
      next unless server['alias']

      # Reject global aliases (See 7513)
      server['alias'].reject { |cname| cname.include?('.') }.each do |cname|
...
def get_pdus_records(site)
  ...
  site['pdus'].sort.each do |pdu_uid, pdu|
...
def get_networks_records(site, key)
  ...
  site[key].sort.each do |uid, net|
    ...
    net['alias'].each do |a|
      ...
      next unless a.is_a?(String) and !a.include?('.') # Reject global aliases (See 7513)
...
def get_node_records(cluster_uid, node_uid, network_adapters)
  ...
  network_adapters.each do |net_uid, net_hash|
    ...
    next unless net_hash['alias']

    net_hash['alias'].each do |cname|
      cname_record = DNS::Zone::RR::CNAME.new
...
def get_node_kavlan_records(_cluster_uid, node_uid, network_adapters, kavlan_adapters)
  ...
  # each network adapters
  kavlan_adapters.each do |net_uid, net_hash|
    next unless net_hash['ip'] || net_hash['ip6']

    net_primaries = # list of primary interfaces
      network_adapters.select do |u, h|
        h['mounted'] && /^eth[0-9]$/.match(u)
      end
    ...
    next unless net_primaries.include?(net_uid_eth) and net_hash['ip6']
...
def get_reverse_record(record, site_uid)
  ...
  if record.is_a?(DNS::Zone::RR::AAAA) # check for AAAA before A because AAAA inherits from A (so an AAAA is also an A)
    ...
    nibble_split = 16
    nibble_split = 14 if /.*-kavlan-[1-9][0-9]-ipv6$/.match(record.label)
...
def include_manual_file(zone, zone_type)
  manual_file_path = File.join(File.dirname(zone.file_path),
                               File.basename(zone.file_path).sub('.db', '') + '-manual.db')
...
def load_zone(zone_file_path, site_uid, site, zone_type, add_header)
  zone = if File.exist?(zone_file_path)
           DNS::Zone.load(File.read(zone_file_path))
         else
           DNS::Zone.new
         end
...
def set_internal_zone_header_records(zone, site)
  ...
  return unless File.basename(zone.file_path) == "#{zone.site_uid}.db" && site['frontend_ip']
...
def update_serial(serial)
  ...
  if date_serial.strftime('%Y%m%d') == now_date.strftime('%Y%m%d')
    date_serial += Rational(1, 86_400) # +1 second
    date_serial.strftime('%Y%m%d%S')
  else
    Time.now.utc.strftime('%Y%m%d00')
  end
end
...
def diff_zone_file(zone, records)
  ...
  zone_records = zone.records.map do |rec|
    rec.dump
  end
  recs = records.map do |rec|
    rec.dump
  end
  ...
  if $options[:verbose]
    if removed_records.any?
      ...
      removed_records.each do |rec|
        puts rec
      end
    end
    if added_records.any?
      puts "Added records in zone file: #{zone.file_path}"
      added_records.each do |rec|
        puts rec
      end
    end
  end
  added_records.any? || removed_records.any?
end
...
def write_site_conf(site_uid, dest_dir, zones_dir)
  ...
  conf_content = ERB.new(File.read(File.expand_path('templates/bind-site.conf.erb',
                                                    File.dirname(__FILE__)))).result(binding)
...
def write_site_local_conf(site_uid, dest_dir, zones_dir)
  ...
  conf_content = ERB.new(File.read(File.expand_path('templates/bind-site-local.conf.erb',
                                                    File.dirname(__FILE__)))).result(binding)
  File.write(conf_file, conf_content)
end
...
def write_site_external_conf(site_uid, dest_dir, zones_dir)
  conf_file = File.join(dest_dir, "#{site_uid}-zones.conf")
  FileUtils.mkdir_p(File.dirname(conf_file))
  conf_content = ERB.new(File.read(File.expand_path('templates/bind-site-external.conf.erb',
                                                    File.dirname(__FILE__)))).result(binding)
  File.write(conf_file, conf_content)
end
...
def fetch_site_records(site, type)
  ...
  site_records['networks'].select! { |record| record.is_a?(DNS::Zone::RR::AAAA) } if type == :external

  # each cluster
  site.fetch('clusters', []).sort.each do |cluster_uid, cluster|
    # each nodes
    cluster.fetch('nodes').select do |_node_uid, node|
      !node.nil? && node['status'] != 'retired' && node.has_key?('network_adapters')
    end.each_sort_by_node_uid do |node_uid, node|
      network_adapters = {}

      # Nodes
      node.fetch('network_adapters').each do |net|
      ...
      kavlan_kinds.each do |kavlan_kind|
        next unless node[kavlan_kind]

        node.fetch(kavlan_kind).each do |net_uid, net_hash|
          net_hash.each do |kavlan_net_uid, ip|
            kavlan_adapters["#{net_uid}-#{kavlan_net_uid}"] ||= {}
            if /^eth[0-9]$/.match(net_uid)
              kavlan_adapters["#{net_uid}-#{kavlan_net_uid}"]['mounted'] = node['network_adapters'].select do |n|
                n['device'] == net_uid
              end[0]['mounted']
              kavlan_adapters["#{net_uid}-#{kavlan_net_uid}"]['pname'] = node['network_adapters'].select do |n|
                n['device'] == net_uid
              end.first['name'] + '-' + kavlan_net_uid
            ...
            next unless /^fpga[0-9]$/.match(net_uid)

            kavlan_adapters["#{net_uid}-#{kavlan_net_uid}"]['mountable'] = node['network_adapters'].select do |n|
              n['device'] == net_uid
            end[0]['moutable']
            if kavlan_kind == 'kavlan6'
              kavlan_adapters["#{net_uid}-#{kavlan_net_uid}"]['ip6'] = ip
      ...
      next unless kavlan_adapters.length > 0
...
def compute_reverse_records(site_uid, site_records)
  ...
  site_records.each do |zone, records|
    # Sort records
    ...
    records.each do |record|
      ...
      next if reverse_file_name.nil?

      reverse_records[reverse_file_name] ||= []
      reverse_records[reverse_file_name].each do |r|
...
def generate_internal_site_data(site_uid, site, dest_dir, zones_dir)
  ...
  reverse_records.each do |file_name, records|
  ...
  site_records.each do |type, records|
  ...
  Dir.glob(File.join(zones_dir, 'reverse-*-manual.db')).each do |reverse_manual_file|
...
def generate_external_site_data(site_uid, site, dest_dir, zones_dir)
  ...
  reverse_records.each do |file_name, records|
    ...
    next unless file_name.start_with?('reverse6')
