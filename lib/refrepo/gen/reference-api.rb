...
def create_network_equipment(network_uid, network, refapi_path, site_uid = nil)
  network_path = if site_uid
                   Pathname.new(refapi_path).join('sites', site_uid, 'network_equipments')
                 else
                   Pathname.new(refapi_path).join('network_equipments')
                 end
  network_path.mkpath
...
def gen_node_description(node)
  ...
  node['storage_devices'].each do |device|
    unless device['storage'].nil?
      if storages[device['storage']].nil?
        storages[device['storage']] =
          [(device['size'].to_i / 1024**3).round]
      else
        storages[device['storage']] += [(device['size'].to_i / 1024**3).round]
      end
    end
  end # each storage
  cluster_storages = ''
  ...
  gpu_message  = if nb_gpu
                   ", #{nb_gpu} GPU#{nb_gpu > 1 ? 's' : ''} #{gpu_model}"
                 else
                   ''
                 end
  "#{nb_cpu} CPU#{nb_cpu > 1 ? 's' : ''} #{proc_model} #{proc_version}, #{core_per_cpu} core#{core_per_cpu > 1 ? 's' : ''}/CPU#{gpu_message}, #{ram}GB RAM#{pmem_message},#{cluster_storages}, #{na.map do |k, v|
                                                                                                                                                                                                   "#{v} x #{k}"
                                                                                                                                                                                                 end.join(', ')}"
end
...
def generate_reference_api
  ...
  if global_hash['uid']
    ...
    write_json(grid_path.join("#{global_hash['uid']}.json"),
               global_hash.reject do |k, _v|
                 %w[sites network_equipments disk_vendor_model_mapping].include?(k)
               end)
  end
  ...
  global_hash['sites'].each do |site_uid, site|
    ...
    write_json(site_path.join("#{site_uid}.json"),
               site.reject do |k, _v|
                 %w[clusters networks pdus dom0 laptops servers sensors].include?(k)
               end)
    ...
    site.fetch('clusters', []).sort.each do |cluster_uid, cluster|
      ...
      #
      cluster['manufactured_at'] = cluster['nodes'].filter do |_n_uid, n_hash|
                                     n_hash.key? 'chassis'
                                   end.map { |_n_uid, n_hash| n_hash['chassis']['manufactured_at'] }.min
      cluster['warranty_end'] = cluster['nodes'].filter do |_n_uid, n_hash|
                                  n_hash.key? 'chassis'
                                end.map { |_n_uid, n_hash| n_hash['chassis']['warranty_end'] }.min
      ...
      if cluster['priority'].nil?
        ...
        cluster['priority'] += 100 if cluster['nodes'].first[1].key?('gpu_devices')
      end
      ...
      cluster['nodes'].each do |node_uid, node| # _sort_by_node_uid
        ...
        node['network_adapters'].each do |key, _hash|
          node['network_adapters'][key]['device'] = key
        end
        node['network_adapters'] =
          node['network_adapters'].sort_by_array(['eth0', 'eth1', 'eth2', 'eth3', 'eth4', 'eth5', 'eth6', 'eth7', 'ib0.8100',
                                                  'ib0', 'ib1', 'ib2', 'ib3', 'ib4', 'ib5', 'ib6', 'ib7', 'ibs1', 'ibs3', 'ibs4', 'ibp130s0', 'bmc', 'eno1', 'eno2', 'eno3', 'eno4', 'eno1np0', 'eno2np1', 'ens4f0', 'ens4f1', 'ens5f0', 'ens5f1', 'ens10f0np0', 'ens10f1np1', 'fpga0', 'fpga1']).values
        # Add "device: dimm_a1" within the hash
        node['memory_devices'].each do |key, _hash|
          node['memory_devices'][key]['device'] = key
        end
        node['memory_devices'] = node['memory_devices'].sort_by do |d, _|
      ...
      first_node = # use first non-retired node
        cluster['nodes'].values.select do |x|
          x['status'] != 'retired'
        end.sort_by { |x| split_cluster_node(x['uid']) }.first
