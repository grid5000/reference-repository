...
def dell_product_data
  ...
  services_tags.each_slice(100).each do |d|
  ...
  data.each do |_s, s_hash|
    s_hash['clusters'].each do |_c, c_hash|
      c_hash['nodes'].each do |_n, info|
        ...
        info['chassis']['warranty_end'] = DateTime.strptime(product['entitlements'].map do |e|
                                                              e['endDate']
                                                            end.max, '%Y-%m-%d').to_date
