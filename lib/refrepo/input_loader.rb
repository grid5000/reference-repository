    ...
    list_of_yaml_files.each { |filename|
      begin
        ...
        if /\.y.*ml\.erb$/.match(filename)
          ...
          file_hash = YAML::load(ERB.new(File.read(filename)).result(binding), permitted_classes: [Date, Time], aliases: true)
        else
          file_hash = YAML::load_file(filename, permitted_classes: [Date, Time], aliases: true)
        end
      if not file_hash
