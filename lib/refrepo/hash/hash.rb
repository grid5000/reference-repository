...
def rec_sort(h)
  ...
  when Hash
    Hash[Hash[h.map { |k, v| [rec_sort(k), rec_sort(v)] }].sort_by do |k, v|
      [begin
        k.to_s
...
def write_json(filepath, data, perm = 0o644)
...
def write_yaml(filepath, data, perm = 0o644)
  File.open(filepath, 'w', perm) do |f|
...
class ::Hash
  def slice(*extract)
    self.select { |key, _value| extract.include?(key) }
  end
  ...
  def expand_square_brackets(keys = clone)
    ...
    keys.each do |key, value|
      self[key].expand_square_brackets(value) if value.is_a?(Hash)
    end
  end
  ...
  def self.from_array(array, value)
    array.reverse.inject(value) { |a, n| { n => a } }
  end

  # Custom iterator. Same as "each" but it sorts keys by node_uid (ie. graphene-10 after graphene-9)
  def each_sort_by_node_uid
    sort_by { |item| item.to_s.split(/(\d+)/).map { |e| [e.to_i, e] } }.each do |key, value|
      yield key, value unless key.nil?
    end
  end
  ...
  def each_filtered_node_uid(cluster_list, node_list)
    each_sort_by_node_uid do |key, properties|
