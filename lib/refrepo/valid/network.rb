  ...
  options[:sites].each do |site|
    ...
    nodes.each do |n|
      n['network_adapters'].each do |nic|
        next unless nic['mounted'] or nic['mountable']
    ...
    neteqs.each do |eq|
      ...
      puts "WARNING: #{eq['uid']} has no backplane_bps" unless eq['backplane_bps']
      next unless eq['linecards'].length > 1 # only look at LC-specific backplane if switch has more than one linecard

      eq['linecards'].each do |lc|
        next if lc.keys.empty?

        puts "WARNING: #{eq['uid']} has a linecard without backplane_bps" unless lc['backplane_bps']
      end
    end
    ...
    neteqs.each do |eq|
      ...
      eq['linecards'].each_with_index do |lc, _lc_i|
        (lc['ports'] || []).each_with_index do |port, _port_i|
          ...
          if port['kind'] == 'node'
            mynetnodes = netnodes.select do |n|
              n.values_at('kind', 'uid', 'port') == port.values_at('kind', 'uid', 'port')
            end
          ...
          elsif %w[switch router].include?(port['kind'])
          ...
          elsif %w[other virtual server backbone].include?(port['kind'])
            ...
            if channel && %w[server other].include?(channel['kind'])
              puts "INFO: skipping channel for #{channel['kind']} #{port['uid']}"
            elsif channel && %w[switch router].include?(channel['kind'])

              select_ports = lambda do |linecards, uid|
                linecards.map do |l|
    ...
    neteqs.each do |eq|
      ...
      eq['channels'].each do |k, v|
        ...
        # only if the channel is connected to router/switch
        next unless %w[switch router].include?(v['kind'])

        c_lookup = neteqs.select { |n| n['uid'] == v['uid'] }
        c_lookup = c_lookup.length.zero? ? nil : c_lookup.first['channels']

        next unless !c_lookup.nil? and !c_lookup[v['port']].nil? and
    ...
    netnodes.select { |n| n['found'] > 1 }.each do |n|
      # routers and switches might be connected several times, that's OK
      next if %w[router switch].include?(n['kind'])
    ...
    linknicks.uniq.each do |l|
      ...
      if %w[router switch].include?(n0['kind']) and %w[router switch].include?(n1['kind'])
    ...
    generate_dot(netnodes, links, site) if options[:dot]
  end
  ok
end
...
def generate_dot(netnodes, links, site)
  ...
  links.delete_if { |l| !(nicknames.include?(l['switch']) and nicknames.include?(l['target'])) }

  # enrich links
  links.each do |l|
    nn = mynetnodes.select { |n| n['nickname'] == l['target'] }.first
    l['target_kind'] = nn['kind']
    l['target_cluster'] = l['target'].split('-')[0] if l['target_kind'] == 'node'
  end

  # remove duplicate reverse links between switches. We keep only the one where the target is the second node.
  links.delete_if { |l| %w[router switch].include?(l['target_kind']) and l['target'] == l['nicknames'].first }

  # separate links to equipments
  eqlinks = links.select { |l| %w[router switch].include?(l['target_kind']) }
  # group links between same pairs of switches
  eqlinks = eqlinks.group_by { |e| e }.to_a.map do |e|
    e[0]['count'] = e[1].length
    e[0]
  end
  ...
  # group
  nodeslinks = nodeslinks.group_by { |l| [l['target_cluster'], l['attachments']] }.to_a.map do |e|
    e[1].map! do |f|
      f['target_node']
    end
    e
  ...
  nodeslinks.map! do |e|
    e[1] = sh("echo #{e[1].uniq.join(' ')}|nodeset -f")
    e
  end
  ...
  mynetnodes.select { |n| %w[router switch].include?(n['kind']) }.sort_by { |e| e['uid'] }.each do |eq|
