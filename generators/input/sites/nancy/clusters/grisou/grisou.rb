  ...
  cluster :grisou do |cluster_uid|
    ...
    51.times do |i|
      node "#{cluster_uid}-#{i+1}" do |node_uid|
        ...
        # Node with 1 interface
        elsif (i != 49 and i != 50) then
        network_adapters [{
          :interface        => lookup(node_uid, node_uid, 'network_interfaces', 'eth0', 'interface'),
          :rate             => lookup(node_uid, node_uid, 'network_interfaces', 'eth0', 'rate'),
          :enabled          => true, 
        ...
        },
        {
          ...
          :hwid             => lookup('grisou_manual', node_uid, 'network_interfaces', 'ib0', 'hwid'),
          :switch           => "sgraoullyib",
          :ib_switch_card   => lookup('grisou_manual', node_uid, 'network_interfaces', 'ib0', 'line_card'),
          :ib_switch_card_pos => lookup('grisou_manual', node_uid, 'network_interfaces', 'ib0', 'position'),
        },]
        end
