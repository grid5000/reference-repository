    ...
    50.times do |i|
      node "#{cluster_uid}-#{i+1}" do |node_uid|
        ...
        network_adapters [{
            ...
            :ip => lookup('sophia-sol', node_uid, 'network_interfaces','eth0', 'ip'),
            :vendor => 'nVidia',
            :version => 'MCP55 Pro',
            :enabled => true,
            :management => false,
            ...
            :switch => lookup('sophia-sol', node_uid, 'network_interfaces','eth0', 'switch_name'),
            :switch_port => lookup('sophia-sol', node_uid, 'network_interfaces','eth0', 'switch_port')
          },{
            :interface => 'Ethernet',
            :rate => 1.G,
            :mac => lookup('sophia-sol', node_uid, 'network_interfaces', 'eth1', 'mac'),
            :vendor => 'nVidia',
            :version => 'MCP55 Pro',
            :enabled => false,
            :driver => 'forcedeth',
         ...
         # }
          {
            ...
            :ip => lookup('sophia-sol', node_uid, 'network_interfaces','bmc', 'ip'),
            :mac => lookup('sophia-sol', node_uid, 'network_interfaces','bmc', 'mac'),
            :enabled  => true,
            :mounted => false,
            :mountable => false,
            :management => true,
          ...
          chassis({
            :serial => lookup('sophia-sol', node_uid, 'chassis','serial_number'),
            :name => lookup('sophia-sol', node_uid, 'chassis','product_name')
          })
      end
    end
  end
